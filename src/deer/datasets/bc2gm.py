...
Please identify "GENE" entities (exact text spans), following the JSON template listed above, and output the JSON object. If no named entities identified, output {"named entities": []}.
...
1. Evaluate each candidate token listed above to determine if it should be categorized as (part of) a "GENE" entity. Consider both the positive and negative examples provided carefully. Pay particular attention to the overall statistical data on whether tokens are included or excluded from the entity. Hard negative examples highlight tokens that are not part of the entity but are located near it.
